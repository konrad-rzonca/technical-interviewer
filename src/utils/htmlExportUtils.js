...
/**
 * Create HTML for the candidate details section
 * @param {Object} metadata - Metadata about the candidate
 * @returns {string} - HTML for the candidate details section
 */
const createCandidateDetailsHTML = (metadata) => {
  const {candidateName, comments, recommendation} = metadata || {};

  // Only return the container if we have at least one piece of information to show
  if (!candidateName && !recommendation && !comments) {
    return '';
  }
 ...
 */
const prepareInterviewDataForExport = (
 ...
 */
const createReportHTML = (exportData, options = {}) => {
  const {timestamp, metadata, data} = exportData;
  const {
    notesMap,
    gradesMap,
    questionDetails,
    ...
    return `
      ...
      <body>
        <div class="empty-container">
          <h1>Technical Interview Notes</h1>
    ...
    <head>
      ...
      : TYPOGRAPHY.fontFamily};
        ...
        .recommendation-icon.negative {
          background-color: ${useUbsTheme ? '#EC0016' : COLORS.error.main};
        }
        ...
        .recommendation-text.negative {
          color: ${useUbsTheme ? '#EC0016' : COLORS.error.main};
        }
...
/**
 * Export interview data as an HTML file or return the HTML content
 * @param {Object} interviewState - Current interview state
 * @param {Array} allQuestions - All available questions
 * @param {Object} metadata - Additional metadata for the report
 * @param {Object} options - Options for the export process
 * @returns {string|boolean} - HTML content or success indicator
 */
export const exportInterviewData = async (
    interviewState, allQuestions, metadata = {}, options = {}) => {
  // Prepare the data for export
  const dataToExport = prepareInterviewDataForExport(interviewState,
      allQuestions, metadata);

  // Generate the HTML content
  const htmlContent = createReportHTML(dataToExport, options);

  // If generateHtmlOnly is true, just return the HTML content without downloading
  if (options && options.generateHtmlOnly) {
    return htmlContent;
  }
